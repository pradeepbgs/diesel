...
`,v.outputFunctionName){if(!y.test(v.outputFunctionName))throw new Error("outputFunctionName is not a valid JS identifier.");u+="  var "+v.outputFunctionName+` = __append;
`}if(v.localsName&&!y.test(v.localsName))throw new Error("localsName is not a valid JS identifier.");if(v.destructuredLocals&&v.destructuredLocals.length){var z="  var __locals = ("+v.localsName+` || {}),
`;for(var J=0;J<v.destructuredLocals.length;J++){var $=v.destructuredLocals[J];if(!y.test($))throw new Error("destructuredLocals["+J+"] is not a valid JS identifier.");if(J>0)z+=`,
  `;z+=$+" = __locals."+$}u+=z+`;
...
`,Z.message+="Or, if you meant to create an async function, pass `async: true` as an option."}throw Z}var A=v.client?g:function Z(L){var B=function(V,c){var M=K.shallowCopy(K.createNullProtoObjWherePossible(),L);if(c)M=K.shallowCopy(M,c);return j1(V,v)(M)};return g.apply(v.context,[L||K.createNullProtoObjWherePossible(),w,B,Af])};if(v.filename&&typeof Object.defineProperty==="function"){var b=v.filename,Q=W.basename(b,W.extname(b));try{Object.defineProperty(A,"name",{value:Q,writable:!1,enumerable:!1,configurable:!0})}catch(Z){}}return A},generateSource:function(){var f=this.opts;if(f.rmWhitespace)this.templateText=this.templateText.replace(/[\r\n]+/g,`
...
`}};bf.escapeXML=K.escapeXML;bf.__express=bf.renderFile;bf.VERSION=of;bf.name=g1;if(typeof window!="undefined")window.ejs=bf});function F(f){switch(f.split(".").pop()?.toLowerCase()){case"js":return"application/javascript";case"css":return"text/css";case"html":return"text/html";case"json":return"application/json";case"png":return"image/png";case"jpg":case"jpeg":return"image/jpeg";case"svg":return"image/svg+xml";case"gif":return"image/gif";case"woff":return"font/woff";case"woff2":return"font/woff2";default:return"application/octet-stream"}}var x=null;async function Q1(){if(!x){let f=await Promise.resolve().then(() => Mf(Uf(),1));x=f.default||f}return x}var G1={string:"text/plain; charset=utf-8",object:"application/json; charset=utf-8",Uint8Array:"application/octet-stream",ArrayBuffer:"application/octet-stream"};class _{req;server;path;routePattern;paramNames;env;executionContext;headers=new Headers;parsedQuery=null;parsedParams=null;parsedCookies=null;parsedBody=null;contextData={};urlObject=null;constructor(f,g,v,u,C,w,r){this.req=f,this.server=g,this.path=v,this.routePattern=u,this.executionContext=r,this.env=w,this.paramNames=C}setHeader(f,g){return this.headers.set(f,g),this}removeHeader(f){return this.headers.delete(f),this}set(f,g){return this.contextData[f]=g,this}get(f){return this.contextData[f]}get ip(){if(this.server)return this.server.requestIP(this.req)?.address??null;return this.req.headers.get("CF-Connecting-IP")||null}get url(){if(!this.urlObject)this.urlObject=new URL(this.req.url);return this.urlObject}get query(){if(!this.parsedQuery)this.parsedQuery=this.url.search?Object.fromEntries(this.url.searchParams):{};return this.parsedQuery}get params(){if(!this.parsedParams)try{this.parsedParams=B1(this.paramNames,this.path)}catch(f){let g=f instanceof Error?f.message:String(f);throw new Error(`Failed to extract route parameters: ${g}`)}return this.parsedParams??{}}get body(){if(this.req.method==="GET")return Promise.resolve({});if(!this.parsedBody)this.parsedBody=(async()=>{try{let f=await X1(this.req);if(f.error)throw new Error(f.error);return Object.keys(f).length===0?null:f}catch(f){throw new Error("Invalid request body format")}})();return this.parsedBody}text(f,g=200){return new Response(f,{status:g,headers:this.headers})}send(f,g=200){let v;if(f instanceof Uint8Array)v="Uint8Array";else if(f instanceof ArrayBuffer)v="ArrayBuffer";else v=typeof f;if(!this.headers.has("Content-Type"))this.headers.set("Content-Type",G1[v]??"text/plain; charset=utf-8");let u=v==="object"&&f!==null?JSON.stringify(f):f;return new Response(u,{status:g,headers:this.headers})}json(f,g=200){return Response.json(f,{status:g,headers:this.headers})}file(f,g,v=200){let u=Bun.file(f);if(!this.headers.has("Content-Type"))this.headers.set("Content-Type",g??F(f));return new Response(u,{status:v,headers:this.headers})}async ejs(f,g={},v=200){let u=await Q1();try{let C=await Bun.file(f).text(),w=u.render(C,g),r=new Headers({"Content-Type":"text/html; charset=utf-8"});return new Response(w,{status:v,headers:r})}catch(C){return console.error("EJS Rendering Error:",C),new Response("Error rendering template",{status:500})}}redirect(f,g=302){return this.headers.set("Location",f),new Response(null,{status:g,headers:this.headers})}setCookie(f,g,v={}){let u=`${encodeURIComponent(f)}=${encodeURIComponent(g)}`;if(v.maxAge)u+=`; Max-Age=${v.maxAge}`;if(v.expires)u+=`; Expires=${v.expires.toUTCString()}`;if(v.path)u+=`; Path=${v.path}`;if(v.domain)u+=`; Domain=${v.domain}`;if(v.secure)u+="; Secure";if(v.httpOnly)u+="; HttpOnly";if(v.sameSite)u+=`; SameSite=${v.sameSite}`;return this.headers.append("Set-Cookie",u),this}get cookies(){if(!this.parsedCookies){let f=this.req.headers.get("cookie");this.parsedCookies=f?b1(f):{}}return this.parsedCookies}stream(f){let g=new Headers(this.headers),v=new ReadableStream({async start(u){await f(u),u.close()}});return new Response(v,{headers:g})}yieldStream(f){return new Response}}function b1(f){return Object.fromEntries(f.split(";").map((g)=>{let[v,...u]=g.trim().split("=");return[v,decodeURIComponent(u.join("="))]}))}function B1(f,g){let v={},[u]=g.split("?"),C=u.split("/").filter((r)=>r!==""),w=C.length-f.length;for(let r=0;r<f.length;r++)v[f[r]]=C[w+r];return v}function O1(f,g){let v={},u=f.split("/"),[C]=g.split("?"),w=C.split("/");if(u.length!==w.length)return null;for(let r=0;r<u.length;r++){let j=u[r];if(j.charCodeAt(0)===58)v[j.slice(1)]=w[r]}return v}async function X1(f){let g=f.headers.get("Content-Type")||"";if(!g)return{};if(f.headers.get("Content-Length")==="0"||!f.body)return{};if(g.startsWith("application/json"))return await f.json();if(g.startsWith("application/x-www-form-urlencoded")){let u=await f.text();return Object.fromEntries(new URLSearchParams(u))}if(g.startsWith("multipart/form-data")){let u=await f.formData(),C={};for(let[w,r]of u.entries())C[w]=r;return C}return{error:"Unknown request body type"}}async function Ff(f,g,v){if(!g?.length)return;for(let u=0;u<g.length;u++){let C=g[u](...v),w=C instanceof Promise?await C:C;if(w&&f!=="onRequest")return w}}async function P1(f,g,v){let u=f.globalMiddlewares;if(u.length)for(let w of u){let r=await w(v);if(r)return r}let C=f.middlewares.get(g);if(C&&C.length)for(let w of C){let r=await w(v);if(r)return r}return null}async function qf(f,g,v){for(let u of f){let C=await u(g,v);if(C)return C}}async function Df(f,g,v){let u=await W1(f,g,v),C=u instanceof Promise?await u:u;if(C)return C}async function W1(f,g,v){if(g.endsWith("/"))g=g.slice(0,-1);if(!f.filters.has(g))if(f.filterFunction.length)for(let u of f.filterFunction){let C=await u(v);if(C)return C}else return Response.json({error:"Protected route, authentication required"},{status:401})}async function Rf(f,g,v){if(f.staticPath){let C=!0;if(f.staticRequestPath)C=v.startsWith(f.staticRequestPath);if(C){let w=await U1(f,v,g);if(w)return w;let r=f.router.find(g.req.method,"*");if(r?.handler)return await r.handler(g)}}let u=f.routeNotFoundFunc(g);return u instanceof Promise?await u:u||T(404,`404 Route not found for ${v}`)}function T(f,g){return new Response(JSON.stringify({error:g}),{status:f,headers:{"Content-Type":"application/json"}})}async function U1(f,g,v){if(!f.staticPath)return null;let u=`${f.staticPath}${g}`;if(await Bun.file(u).exists()){let w=F(u);return v.file(u,w,200)}return null}var N=(f)=>f.constructor.name==="AsyncFunction",k=(f,g,v,...u)=>{if(g.length>5)f.push(`
  ...
  `)},q1=(f,g)=>{if(g.length<=5)for(let v=0;v<g.length;v++)if(N(g[v]))f.push(`
  ...
  `)},k1=(f,g)=>{let v=[],u=g.globalMiddlewares||[],C=f?.hasOnReqHook?g.hooks.onRequest:[],w=f?.hasPreHandlerHook?g.hooks.preHandler:[],r=f?.hasOnSendHook?g.hooks.onSend:[];if(F1(v),v.push(`
    ...
    `),f?.hasMiddleware){if(u.length>0)q1(v,u);if(g.middlewares.size>0)v.push(`
...
`)}
  ...
  `;return new Function("executeBunMiddlewares","handlers","runHooks","filterFunctions","onRequestHooks","allMiddlewares",Z)(qf,j,Ff,Q,A,$)};export{k1 as buildRequestPipeline,m1 as BunRequestPipline};
